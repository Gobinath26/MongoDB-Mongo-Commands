...
//exception- for except _id
db.movies.find({}, { _id: 0, name: 1, rating: 1 }).pretty();
//Select name,rating from movies where rating > 8;|without _id in output
db.movies.find({ rating: { $gt: 8 } }, { _id: 0, name: 1, rating: 1 }).pretty();
//Sorting - asc by rating
db.movies.find({}).sort({ rating: 1 }).pretty();
//Select name,rating from movies order by rating|without _id in output
db.movies.find({}, { _id: 0, name: 1, rating: 1 }).sort({ rating: 1 }).pretty();
//Select name,rating from movies order by rating in desc|without _id in output
db.movies
  .find({}, { _id: 0, name: 1, rating: 1 })
  .sort({ rating: -1 })
  .pretty();
//Select name,rating from movies order by rating limit 2
db.movies
  .find({}, { _id: 0, name: 1, rating: 1 })
  .sort({ rating: 1 })
  .limit(2)
...
db.movies
  ...
  .skip(2)
  .pretty();
//sort alphabaticaly when sorted rating is equal
db.movies
  .find({}, { _id: 0, name: 1, rating: 1 })
...
//to check
db.movies.find({}, { _id: 0, name: 1, language: 1 });
//Select/find all movies with rating =8 and language is english
db.movies.find({rating:8,language:"english"},{ _id: 0, name: 1, language: 1, rating:1})
//-------Delete-------
//find
db.movies.find({rating:8,language:"english"},{ _id: 0, name: 1, language: 1, rating:1})
//delete
db.movies.deleteMany({rating:8,language:"english"})
//to check
db.movies.find({}).count()
